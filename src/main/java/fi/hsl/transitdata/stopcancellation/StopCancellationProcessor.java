...
public class StopCancellationProcessor {
    ...
    private Map<String, List<InternalMessages.StopCancellations.StopCancellation>> stopCancellationsByStopId = Collections
            .emptyMap();
    ...
    public StopCancellationProcessor(ZoneId timezone, Duration tripWithTripUpdateCacheDuration,
            Duration tripWithCancellationCacheDuration) {
    ...
    public void updateStopCancellations(InternalMessages.StopCancellations stopCancellations) {
        this.stopCancellationsByStopId = stopCancellations.getStopCancellationsList().stream()
                .collect(Collectors.groupingBy(InternalMessages.StopCancellations.StopCancellation::getStopId));
        this.journeyPatternById = stopCancellations.getAffectedJourneyPatternsList().stream()
        ...
        stopCancellations.getStopCancellationsList().forEach(stopCancellation -> {
            if (stopCancellation.getCause() == InternalMessages.StopCancellations.Cause.JOURNEY_DETOUR) {
                ...
                journeyPatternIdByTripIdentifier.put(TripIdentifier.fromTripInfo(stopCancellation.getAffectedTrip()),
                        stopCancellation.getAffectedJourneyPatternIds(0));
            } else {
                ...
                stopCancellation.getAffectedJourneyPatternIdsList().stream().map(journeyPatternById::get)
                        .forEach(journeyPattern -> {
                            journeyPattern.getTripsList().forEach(tripInfo -> {
                                ...
                                if (tripStartTime.isAfter(from) && tripStartTime.isBefore(to)) {
                                    journeyPatternIdByTripIdentifier.put(TripIdentifier.fromTripInfo(tripInfo),
                                            journeyPattern.getJourneyPatternId());
                                }
                            });
                        });
            }
    ...
    private Collection<TripUpdateWithId> getTripUpdatesForCancellationsOfCancellations(final long timestamp) {
        return tripsWithCancellations.asMap().entrySet().stream()
                ...
                .filter(tripIdentifierAndId -> !journeyPatternIdByTripIdentifier
                        .containsKey(tripIdentifierAndId.getKey()))
                //Trip has not sent trip updates (which would effectively cancel cancellations)
                .filter(tripIdentifierAndId -> !Boolean.TRUE
                        .equals(tripsWithTripUpdates.getIfPresent(tripIdentifierAndId.getKey())))
                .map(tripIdentifierAndId -> {
                    //Create trip update that cancels all stop cancellations
                    GtfsRealtime.TripUpdate tripUpdate = GtfsRealtime.TripUpdate.newBuilder().setTimestamp(timestamp)
                            .setTrip(tripIdentifierAndId.getKey().toGtfsTripDescriptor())
                            .addStopTimeUpdate(GtfsRealtime.TripUpdate.StopTimeUpdate.newBuilder().setStopSequence(1)
    ...
    private Collection<TripUpdateWithId> getTripUpdatesForFutureStopCancellations(final long timestamp) {
        return journeyPatternById.values().stream().map(journeyPattern -> {
            ...
            final List<GtfsRealtime.TripUpdate.StopTimeUpdate> stopTimeUpdates = journeyPattern.getStopsList().stream()
                    ...
                    .map(StopCancellationProcessor::createNoDataStopTimeUpdate).collect(Collectors.toList());
            ...
            for (InternalMessages.TripInfo tripInfo : journeyPattern.getTripsList()) {
                ...
                final Collection<GtfsRealtime.TripUpdate.StopTimeUpdate> stopTimeUpdatesWithCancellations = stopTimeUpdates
                        .stream().map(stopTimeUpdate -> {
                            final boolean isStopCancelled = stopCancellationsByStopId
                                    .getOrDefault(stopTimeUpdate.getStopId(), Collections.emptyList()).stream()
                                    .anyMatch(isTripAffectedByStopCancellation(tripIdentifier,
                                            journeyPatternIdByTripIdentifier.get(tripIdentifier)));

                            return isStopCancelled
                                    ? stopTimeUpdate.toBuilder()
                ...
                final boolean hasStopCancellations = stopTimeUpdatesWithCancellations.stream()
                        .map(GtfsRealtime.TripUpdate.StopTimeUpdate::getScheduleRelationship).anyMatch(
                                scheduleRelationship -> scheduleRelationship == GtfsRealtime.TripUpdate.StopTimeUpdate.ScheduleRelationship.SKIPPED);
                ...
                if (hasStopCancellations) {
                    ...
                    final GtfsRealtime.TripUpdate tripUpdate = GtfsRealtime.TripUpdate.newBuilder()
                            .setTrip(tripDescriptor).setTimestamp(timestamp)
                            .addAllStopTimeUpdate(stopTimeUpdatesWithCancellations).build();
    ...
    public Collection<TripUpdateWithId> getStopCancellationTripUpdates(final long timestamp) {
        Collection<TripUpdateWithId> tripUpdatesWithStopCancellations = getTripUpdatesForFutureStopCancellations(
                timestamp);

        Collection<TripUpdateWithId> cancellationsOfCancellations = getTripUpdatesForCancellationsOfCancellations(
                timestamp);
        ...
        tripUpdates.sort(Comparator
                .comparing(tripUpdateWithId -> TripInfoUtils.getStartTime(tripUpdateWithId.tripUpdate.getTrip())));
        ...
        LOG.info(
                "Created {} trip updates for future stop cancellations and {} trip updates for cancellations of stop cancellations",
                tripUpdatesWithStopCancellations.size(), cancellationsOfCancellations.size());
    ...
    public GtfsRealtime.TripUpdate applyStopCancellations(GtfsRealtime.TripUpdate tripUpdate) {
        ...
        final Map<String, GtfsRealtime.TripUpdate.StopTimeUpdate> stopTimeUpdates = tripUpdate.getStopTimeUpdateList()
                .stream()
        ...
        final List<GtfsRealtime.TripUpdate.StopTimeUpdate> stopTimeUpdateList = journeyPattern.getStopsList().stream()
                .sorted(Comparator.comparingInt(InternalMessages.JourneyPattern.Stop::getStopSequence)).map(stop -> {
                    final GtfsRealtime.TripUpdate.StopTimeUpdate stopTimeUpdate = stopTimeUpdates
                            .containsKey(stop.getStopId())
                                    ? stopTimeUpdates.get(stop.getStopId())
                                    : createNoDataStopTimeUpdate(stop.getStopId());

                    final boolean stopCancelled = stopCancellationsByStopId
                            .getOrDefault(stop.getStopId(), Collections.emptyList()).stream()
                    ...
                    if (stopCancelled) {
                        ...
                        return stopTimeUpdate.toBuilder().setScheduleRelationship(
                                GtfsRealtime.TripUpdate.StopTimeUpdate.ScheduleRelationship.SKIPPED).build();
                    } else {
    ...
    private Predicate<InternalMessages.StopCancellations.StopCancellation> isTripAffectedByStopCancellation(
            final TripIdentifier tripIdentifier, final String journeyPatternId) {
    ...
    private static GtfsRealtime.TripUpdate.StopTimeUpdate createNoDataStopTimeUpdate(final String stopId) {
        return GtfsRealtime.TripUpdate.StopTimeUpdate.newBuilder().setStopId(stopId)
                .setScheduleRelationship(GtfsRealtime.TripUpdate.StopTimeUpdate.ScheduleRelationship.NO_DATA).build();
    }

    private static GtfsRealtime.TripUpdate.StopTimeUpdate createSkippedStopTimeUpdate(final String stopId) {
        return GtfsRealtime.TripUpdate.StopTimeUpdate.newBuilder().setStopId(stopId)
                .setScheduleRelationship(GtfsRealtime.TripUpdate.StopTimeUpdate.ScheduleRelationship.SKIPPED).build();
    }
    ...
    private static class TripIdentifier {
        ...
        public static TripIdentifier fromTripInfo(final InternalMessages.TripInfo tripInfo) {
            return new TripIdentifier(RouteIdUtils.normalizeRouteId(tripInfo.getRouteId()), tripInfo.getOperatingDay(),
                    tripInfo.getStartTime(), PubtransFactory.joreDirectionToGtfsDirection(tripInfo.getDirectionId()));
        }

        public static TripIdentifier fromGtfsTripDescriptor(final GtfsRealtime.TripDescriptor tripDescriptor) {
            return new TripIdentifier(tripDescriptor.getRouteId(), tripDescriptor.getStartDate(),
                    tripDescriptor.getStartTime(), tripDescriptor.getDirectionId());
        }

        public GtfsRealtime.TripDescriptor toGtfsTripDescriptor() {
            return GtfsRealtime.TripDescriptor.newBuilder().setRouteId(routeId).setStartDate(operatingDate)
                    .setStartTime(startTime).setDirectionId(directionId).build();
        }
        ...
        public boolean equals(Object o) {
            ...
            return directionId == that.directionId && Objects.equals(routeId, that.routeId)
                    && Objects.equals(operatingDate, that.operatingDate) && Objects.equals(startTime, that.startTime);
        }
